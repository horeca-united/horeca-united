...
const Dashboard = {
  ...
  async render(){
    ...
    const fullBody = rows.map(r=>`
      <tr><td><strong>${r.name}</strong></td><td>${safe(r.supplier)}${r.nextYearAmount != null ? `<br><details style="margin-top:7px"><summary class="btn btn-ghost btn-sm" style="display:inline-block;cursor:pointer">Contractdetails</summary><div style="margin-top:8px;font-size:12px;line-height:1.6;min-width:180px"><strong>1e contractjaar:</strong> ${nlDate(r.contractYears?.firstStart)} t/m ${nlDate(r.contractYears?.firstEnd)}<br><strong>2e contractjaar:</strong> ${nlDate(r.contractYears?.secondStart)} t/m ${nlDate(r.contractYears?.secondEnd)}<br><strong>Indicatief 2e jaar:</strong> ${new Intl.NumberFormat('nl-NL',{style:'currency',currency:'EUR'}).format(r.nextYearAmount)}${r.contractYears?.estimated ? '<br>Start op basis van besteldatum; activatie nog te bevestigen.' : ''}</div></details>` : ''}</td><td>${r.cost == null ? '—' : new Intl.NumberFormat('nl-NL',{style:'currency',currency:'EUR'}).format(r.cost)}${r.id === 'wijn' ? `<br><button class="btn btn-ghost btn-sm" style="margin-top:7px" onclick="Dashboard.openWineDetail()">Bekijk inkoop per leverancier →</button>` : ''}</td><td>${r.sourceYear || '—'}${r.sourceYear === 2026 ? ' · tijdelijk' : ''}</td>
