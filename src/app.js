...
const Dashboard = {
  ...
  async render(){
    ...
    const fullBody = rows.map(r=>`
      <tr><td><strong>${r.name}</strong></td><td>${safe(r.supplier)}${r.id === 'afval' && r.cost != null ? `<br><details style="margin-top:7px"><summary class="btn btn-ghost btn-sm" style="display:inline-block;cursor:pointer">Prijsvergelijking 2025–2026</summary><div style="margin-top:8px;font-size:12px;line-height:1.6;min-width:210px">2025 Q4: € 428,16 excl. btw (3 maanden; € 142,72 abonnement per maand).<br>2026 Q2: € 489,75 excl. btw (3 maanden; € 152,57 abonnement per maand + € 32,04 CO₂- en brandstofheffing).<br><strong>Abonnementstarief: +6,90%.</strong> Totale kwartaalfactuur: +14,39% (€ 61,59).<br>2026 is alleen een benchmark en telt niet mee bij de 2025-kosten. Beide perioden betreffen een 1100L-restafvalcontainer met wekelijkse lediging. Dit zijn kwartaalbedragen, geen jaarbedragen.</div></details>` : ''}</td><td>${r.cost == null ? '—' : new Intl.NumberFormat('nl-NL',{style:'currency',currency:'EUR'}).format(r.cost)}${r.id === 'wijn' ? `<br><button class="btn btn-ghost btn-sm" style="margin-top:7px" onclick="Dashboard.openWineDetail()">Bekijk inkoop per leverancier →</button>` : ''}</td><td>${r.sourceYear || '—'}${r.id === 'afval' && r.sourceYear === 2025 ? ' · Q4' : ''}${r.sourceYear === 2026 ? ' · tijdelijk' : ''}</td>
